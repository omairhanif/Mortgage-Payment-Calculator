...
function parseRatesResponse(content: unknown): { location: MortgageRatesResult["location"]; rates: MortgageRateQuote[] } {
  ...
  const rates = parsed.rates.filter((quote): quote is MortgageRateQuote => {
    if (!quote || typeof quote !== "object") return false;
    const candidate = quote as Record<string, unknown>;
    return typeof candidate.lender === "string" && typeof candidate.rate === "number" && Number.isFinite(candidate.rate) && typeof candidate.apr === "number" && Number.isFinite(candidate.apr) && typeof candidate.payment === "number" && Number.isFinite(candidate.payment);
  });
    ...
    body: JSON.stringify({
      ...
      response_format: { type: "json_schema", json_schema: { name: "mortgage_rates", strict: true, schema: {
        type: "object", additionalProperties: false, required: ["location", "rates"], properties: {
          ...
          rates: { type: "array", minItems: 1, items: { type: "object", additionalProperties: false, required: ["lender", "rate", "apr", "payment"], properties: { lender: { type: "string" }, rate: { type: "number" }, apr: { type: "number" }, payment: { type: "number" } } } },
        },
      } } },
      messages: [{ role: "user", content: `Use web search to retrieve current mortgage rate, APR, and monthly payment offers published by real US lenders as of ${new Date().toISOString().slice(0, 10)}. Do not invent, estimate, calculate, or return generic/plausible rates, APRs, or payments. The payment must be the lender's published payment for the applicable offer, not a payment calculated from the rate. Determine the US city and state for ZIP ${input.zipCode}. Use every field in this exact borrower profile when selecting applicable offers: ${JSON.stringify(borrowerProfile)}. A changed value in any field must be reflected in the returned rates. Return 9 lender offers only when supported by current web sources; otherwise return the fewer supported offers. Keep the response limited to the required JSON fields.` }],
    }),
    cache: "no-store",
  });
